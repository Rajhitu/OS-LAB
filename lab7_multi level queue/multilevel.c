...
{ // Initializing 3 queues
    ...
    int no_of_processes, priority_of_process, burst_time_of_process; // Entering Processes and assigning it to respective queues.
    ...
    while (notComplete(q))
    {
        ...
        {
            l = l % 3;
        } // Process lth queue if its already not executed // If its executed change the value of l
        if (q[l].executed == 1)
        {
            ...
            continue;
        } // Finally you now have a queue which is not completely executed // Process the incomplete processes over it
        if (l == 0)
        {
